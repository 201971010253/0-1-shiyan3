...
if __name__ == '__main__':
    ...
    # 选择数据集
    Lname = tkinter.Label(win, text='输入文件名', font=('华文隶书', 20), fg='skyblue')
    ...
    # 选择第几组数据
    Lph = tkinter.Label(win, text='输入数据编号（0-9）', font=('华文隶书', 20), fg='skyblue')
    ...
    # 单选按钮
    choose = [('1.查询数据', 1), ('2.绘制散点图', 2), ('3.对数据排序', 3), ('4.选择算法求解', 4), ('5.保存算法实验日志数据', 5)]
    v = tkinter.IntVar()
    v.set(1)
    x, y = 0.5, 0.5,
    for a, b in choose:
        cc = tkinter.Radiobutton(win, text=a, variable=v, value=b, font=('华文隶书', 20), fg='skyblue')
